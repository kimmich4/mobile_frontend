...
Foods the user does not like and MUST be avoided: ${dislikedFoods || 'None'}.
...
5. Ensure the diet plan is safe for the provided allergies/conditions, MUST avoid the user's disliked foods, and MUST NOT violate any of the vector database constraints.
6. Formal TITLES: Use different meal names.
7. Portions (grams/ml) must be realistic and specific.
8. Every meal item MUST be a real food or drink with a portion. Do not use filler names like "Calorie balance portion", "extra calories", "calorie filler", "macro adjustment", or any placeholder item. All foods must be generated by you in the original meal plan.

THINKING STEP:
Before writing each day, decide on the number of meals (5 and above) and use formal titles. Then choose real foods with realistic portions and mentally calculate the calories for each so the total matches exactly ${targetCalories}.
